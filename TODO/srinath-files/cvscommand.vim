...
function! s:CVSAnnotate(...)
  ...
  let resultBuffer=s:CVSCreateCommandBuffer('cvs -q annotate -r ' . revision . ' "' . realFileName . '"', 'cvsannotate', '', cvsBufferCheck) 
  if resultBuffer !=  -1
    exec currentLine
...
function! s:CVSCommit()
  ...
  if s:CVSGetOption('CVSCommitOnWrite', 1) == 1
    execute 'au BufWritePre' messageFileName 'g/^CVS:/d'
  ...
  if s:CVSEditFile(messageFileName, cvsBufferCheck) == -1
    ...
    if s:CVSGetOption('CVSCommitOnWrite', 1) == 0
      execute 'nnoremap <buffer> <leader>cc '.
            \ ':g/^CVS:/d<CR>'.
            \ ':update<CR>'.
    ...
    if s:CVSGetOption('CVSCommitOnWrite', 1) == 0 
      put ='CVS: Type <leader>cc to finish this commit operation'
    endif
    put ='CVS: ----------------------------------------------------------------------'
    $
    let b:CVSSourceFile=fileName
